...
PasswordValidator.repeat = (controlName, matchingControlName) => (control) => {
    const input = control.get(controlName);
    const matchingInput = control.get(matchingControlName);
    if (input === null || matchingInput === null) {
        return null;
    }
    if ((matchingInput === null || matchingInput === void 0 ? void 0 : matchingInput.errors) && !matchingInput.errors.repeated) {
        return null;
    }
    if (input.value !== matchingInput.value) {
        matchingInput.setErrors({ repeat: false });
