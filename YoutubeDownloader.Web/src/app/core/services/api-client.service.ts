...
export class ApiClientService {
  ...
  post<T>(uriTemplate: string, commandParameters?: CommandParameters): Observable<T> {
  ...
  postBlob(uriTemplate: string, commandParameters?: CommandParameters): Observable<Blob> {
    const url: string = this.buildUrl(uriTemplate, commandParameters?.segmentParams);
    return this._http.post(url, commandParameters?.data, { responseType: "blob" });
  }

  put<T>(uriTemplate: string, commandParameters?: CommandParameters): Observable<T> {
    const url: string = this.buildUrl(uriTemplate, commandParameters?.segmentParams);
    return this._http.put<T>(url, commandParameters?.data);
  }

  patch<T>(uriTemplate: string, commandParameters?: CommandParameters): Observable<T> {
    const url: string = this.buildUrl(uriTemplate, commandParameters?.segmentParams);
    return this._http.patch<T>(url, commandParameters?.data);
  }

  delete<T>(uriTemplate: string, deleteParameters?: DeleteParameters): Observable<T> {
    const url: string = this.buildUrl(uriTemplate, deleteParameters?.segmentParams);
    return this._http.delete<T>(url);
  }

  private buildUrl(uriTemplate: string, segmentParams?: DataParam, queryParams?: DataParam): string {
