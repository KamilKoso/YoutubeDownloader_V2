...
interface IdentityServerRegisterCredentials {
  email: string;
  username: string;
  password: string;
}
...
export class AuthService {
  ...
  register(credentials: IdentityServerRegisterCredentials): Observable<void> {
    return this._http
      .post<{ userId: string }>(`${this._identityUrl}/register`, credentials)
      .pipe(
        switchMap((result: { userId: string }) =>
          this._http.post<void>(`${appConfig.apiUrl}/user/register`, { id: result.userId, name: credentials.username })
        )
      );
  }
